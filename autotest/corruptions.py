...
def frost(x, severity=1):
    ...
    filename = [resource_filename(__name__, 'frost1.png'),
                resource_filename(__name__, 'frost2.png'),
                resource_filename(__name__, 'frost3.png'),
                resource_filename(__name__, 'frost4.jpg'),
                resource_filename(__name__, 'frost5.jpg'),
                resource_filename(__name__, 'frost6.jpg')][idx]
...
def occlusion(image, severity=1):
    image = asarray(image)
    h, w, _ = image.shape
    left_h = random.randint(0, int(h - h / 3))
    left_w = random.randint(0, int(w - w / 3))
    right_h = left_h + h / 8 * (severity / 2 + 2)
    if right_h >= h:
        right_h = h - 1
    right_w = left_w + w / 8 * (severity / 2 + 2)
    if right_w >= w: -1
    image = cv2.rectangle(image, (left_w, left_h), (int(right_w), int(right_h)), (0, 0, 0), cv2.FILLED)
    return image


def affine(image, severity=1):
    image = asarray(image)
    h, w, _ = image.shape
    mat_src = np.float32([[0, 0], [0, h - 1], [w - 1, 0]])
    mat_dst = np.float32([[random.randint(0, int(w / 8)) * severity, random.randint(0, int(h / 8)) * severity],
                          [random.randint(0, int(w / 8)) * severity, h - random.randint(0, int(h / 8)) * severity],
                          [w - random.randint(0, int(w / 8)) * severity, random.randint(0, int(h / 8)) * severity]])
    mat_trans = cv2.getAffineTransform(mat_src, mat_dst)
    image = cv2.warpAffine(image, mat_trans, (w, h))
    return image


def rotation(image, severity=1):
    image = asarray(image)
    h, w, _ = image.shape
    center = (w // 2, h // 2)
    angle = random.randint(-int(360 / 10 * severity), int(360 / 10 * severity))
