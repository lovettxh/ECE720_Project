...
class autotest():
    ...
    def run_adv_mode(self, x, y, batch_size, com_num, epsilon = 0.03):
        apgd = APGD_attack(self.model, self.device, self.steps, epsilon)
        fab = FAB_Attack_PT(self.model, n_restarts=5, n_iter=self.steps, eps=epsilon, device=self.device)
        c_a = Common_attack(1)
        robust_flags = torch.zeros(x.shape[0], dtype=torch.bool, device=self.device)
        ...
        for idx in range(batch_num):
            ...
            if len(x_.shape) == 3:
                x_.unsqueeze_(dim=0)
            corrupt_image = torch.tensor(c_a.eval(x_.cpu().detach().numpy(), com_num)).type(torch.FloatTensor).to(self.device)
            output = self.model(corrupt_image).max(dim=1)[1]
            false_batch = ~y_.eq(output).to(robust_flags.device)
            non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
            robust_flags[non_robust_lin_idcs] = False
            num_non_robust_batch = torch.sum(false_batch)
            print('{} - {}/{} - {} out of {} successfully perturbed'.format('common-'+str(com_num), idx + 1, batch_num, num_non_robust_batch, x_.shape[0]))
            # update num
        num_robust = torch.sum(robust_flags).item()
        robust_accuracy = torch.sum(robust_flags).item() / x.shape[0]
        print(('robust accuracy after {}: {:.2%} '.format('common-'+str(com_num), robust_accuracy)))
    ...
    def run_test(self, x, y, mode, epsilon = 0.003 , batch_size = 200):
        com_num = [0,1,2,3,4,5,6,13,14,15,16]
        #com_num = com_num[::-1]
        if mode == 'natural':
            self.run_normal_mode(x, y, batch_size, com_num, 0.003)
        elif mode == 'adv':
            self.run_adv_mode(x,y, batch_size, [0], epsilon)
